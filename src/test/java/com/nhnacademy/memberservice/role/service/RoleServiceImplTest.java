...
class RoleServiceImplTest {
    ...
    void testUpdateRole_success() {
        // given
        RoleUpdateRequest request = new RoleUpdateRequest("testName", "수정된 설명");
        when(roleRepository.findById(1L)).thenReturn(Optional.of(role));
    ...
    void testUpdateRole_notFound() {
        // given
        RoleUpdateRequest request = new RoleUpdateRequest("testName", "설명 없음");
        when(roleRepository.findById(999L)).thenReturn(Optional.empty());

        // then
        assertThrows(RoleNotFoundException.class, () -> roleService.updateRole(999L, request));
    }
}